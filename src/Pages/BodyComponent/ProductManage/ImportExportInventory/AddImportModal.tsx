...
import type { OtherFeesType } from "./ImportExportInventory";
import type { ImportRecord, ImportProductDetailsType } from "../../../../zustand/importExportStore";
...
type Props = {
  open: boolean;
  onClose: () => void;
  defaultValues?: Partial<AddImportForm>;
};
const ShippingStatus_Options = ["On Delivery", "Received", "Delay", "Lost", "Police Catched"];

const localTime = new Date().toISOString().slice(0, 10);
const DefaultOtherFee = { value: 0, usedFor: "", date: localTime };
export default function AddImportModal({ open, onClose, defaultValues = {} }: Props) {
  const addRecord = useImportExportStore((s) => s.addRecord);
  const [loading, setLoading] = useState(false);
  const [arrayOtherFees, setArrayOtherFees] = useState<OtherFeesType[]>([DefaultOtherFee]);
  const [sizeAvailable, setSizeAvailable] = useState<string[]>([]);
  const [colorAvailable, setColorAvailable] = useState<string[]>([]);
  const [productDetails, setProductDetails] = useState<ImportProductDetailsType[]>([]);
  const [form, setForm] = useState<AddImportForm>({
    ...
    shippingFeeInternal: defaultValues.shippingFeeInternal ?? 0,
    shippingFeeExternal: defaultValues.shippingFeeExternal ?? 0,
  ...
  React.useEffect(() => {
    if (open) {
      setForm((f) => ({
        ...
        shippingFeeInternal: defaultValues.shippingFeeInternal ?? f.shippingFeeInternal,
        shippingFeeExternal: defaultValues.shippingFeeExternal ?? f.shippingFeeExternal,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    e?.preventDefault();
    if (!form.productId || !form.productName || productDetails.length === 0) {
    ...
    // Calculate brea-even price
    const newProductDetails = [...productDetails];
    productDetails.forEach((d, i) => {
      const initialPercent = (d.importQuantity * d.price) / totalShipment;
      const breakEvenPrice = (totalCost * initialPercent) / d.importQuantity;
      const breakEvenPriceFix = Math.ceil(breakEvenPrice / 1000) * 1000;
      newProductDetails[i].breakEvenPrice = breakEvenPriceFix;
    });
    ...
    try {
      const dataSend: Omit<ImportRecord, "_id"> = {
        ...
        importDetails: newProductDetails,
        sizeAvailable: sizeAvailable,
        colorAvailable: colorAvailable,
        warehouseName: form.warehouseName,
      };
      // console.log('form.warehouseName', form.warehouseName);
      await addRecord("import", dataSend);
      setLoading(false);
      onClose();
    } catch (err) {
  ...
  const handleAddDetail = () => {
    setProductDetails((prev) => [
      ...prev,
      { name: "", stock: 0, color: "", size: "", price: 0, weight: 0, importQuantity: 0, brokenQuantity: 0, addStock: 0, breakEvenPrice: 0 },
    ]);
  };
    ...
    <div className={cx("modal-backdrop")} role="dialog" aria-modal="true" aria-label="Add import record">
      <div className={cx("modal-box")}>
        ...
        <form className={cx("modal-form")} onSubmit={handleSubmit}>
          ...
          <label>
            Warehouse location
            <input type="text" value={form.warehouseName} onChange={(e) => handleChange("warehouseName", e.target.value)} />
          </label>
          <label>
            Shipment status
            ...
            <select>
              <option>None</option>
              {ShippingStatus_Options.map((status, l) => {
                return <option key={l}>{status}</option>;
              })}
            </select>
          </label>
