...
export default function AllOrders_v2() {
  ...
  const defaultNewOrder: FinalOrder = {
  ...
  }, [rawOrders, productDetail, defaultVirtualCart]); // re-run when data/orders or product change

  const sortedOrdersByTime = sortOrders(orders, sortBy);
  const handleSave = async () => {
    if (!editing) return;
    if (currentEditIndex === null) return;
    const dataOrder = rawOrders[currentEditIndex];
  ...
  return filteredConfirmedOrders.filter(
    (o) =>
      ...
      (o.deliveryCode && o.deliveryCode.toLowerCase().includes(lowerText)) // in case you also want to match deliveryCode
  );
};
